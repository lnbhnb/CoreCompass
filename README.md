...
里程碑完成由**任务审阅流程驱动**：队员提交产物 → 系统自动运行产物校验（SQL/MD/Code/JSON/YAML）→ 校验结果展示在任务卡片上 → 队长参考校验结果审阅通过/打回 → 任务自动 `done` → 系统检查该里程碑下所有任务是否全部完成 → 是则里程碑自动 `done` → 所有里程碑完成则项目自动 `completed`。
...
APScheduler 定时扫描逾期任务，按项目聚合后通过飞书 webhook **主动推送**。通知采用**分级冷却策略**避免消息轰炸：
...
- **手动**：看板通知日志区「立即扫描逾期」或 `/api/notify/scan` 可随时手动触发，不受冷却限制
- **即时**：重规划应用后立即推送，不受冷却限制
- **消息格式**：每条通知显示负责人、deadline、已逾期天数、下次巡检时间，评委一眼看懂推送逻辑
